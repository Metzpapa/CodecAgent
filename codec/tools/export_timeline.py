...
class ExportTimelineArgs(BaseModel):
    ...
    output_filename: str = Field(
        "codec_edit.otio",
        description="The desired filename for the timeline file. The extension determines the format: '.otio' (recommended) or '.xml' (legacy)."
    )
    consolidate: bool = Field(
        True,
        description="Creates a self-contained project folder with the timeline file and copies of all used media. This is highly recommended."
    )
...
class ExportTimelineTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Exports the current timeline structure into an editable project file (.otio or .xml) and optionally gathers all used media into a self-contained folder. "
            "This is used to produce the final project file deliverable. After calling this, you should typically call `finish_job` with the returned filename in the `attachments` list."
        )
    ...
    def execute(self, state: 'State', args: ExportTimelineArgs, client: openai.OpenAI, tmpdir: str) -> str:
        if not state.timeline:
            return "Error: Cannot export because the timeline is empty."
        ...
        try:
            ...
            if not output_dir.is_dir():
                return f"Error: Could not find the job's output directory at '{output_dir}'."

            final_relative_path: Path
            base_path_for_relinking: Path
            ...
            if args.consolidate:
                ...
                final_relative_path = Path(package_name) / args.output_filename
                base_path_for_relinking = package_dir
            else:
                final_timeline_file_path = output_dir / args.output_filename
                final_relative_path = Path(args.output_filename)
            ...
            # Return the "Golden" success message with the relative path for the agent to use
            return f"Successfully exported timeline to '{final_relative_path.as_posix()}' in the output directory."

        except Exception as e:
            error_msg = f"An error occurred during timeline export: {e}"
    ...
    def _create_otio_clip(self, codec_clip: TimelineClip, timeline_fps: float, base_path_for_relinking: Path, consolidated: bool) -> otio.schema.Clip:
        ...
        if consolidated:
            ...
            # Use os.path.relpath for robust relative path generation
